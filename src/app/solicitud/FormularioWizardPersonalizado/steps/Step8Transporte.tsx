...
export default function Step8Transporte({
  form,
  update,
  next,
  back,
}: StepPropsWithUpdate) {
  const opciones = ["Coche", "Moto", "Avión", "Barco", "Autocaravana"];

  const toggle = (op: string) => {
    const arr = form.transporte || [];
    if (arr.includes(op)) {
      update("transporte", arr.filter((x) => x !== op));
    } else {
      update("transporte", [...arr, op]);
    }
  };
  ...
  return (
    <div className="space-y-6 animate-fade-slide">
      <h2 className="text-2xl font-semibold text-white">
        ¿Qué transporte prefieres para tu viaje?
      </h2>
      ...
      <div className="space-y-3">
        {opciones.map((op) => (
          <label key={op} className="flex items-center gap-3 text-white">
            <input
              type="checkbox"
              checked={form.transporte?.includes(op)}
              onChange={() => toggle(op)}
              className="w-5 h-5"
            />
            {op}
        ...
        {otroActivo && (
          <input
            type="text"
            placeholder="Especifica tu transporte preferido"
            className="w-full p-3 rounded-xl bg-white/20 border border-white/30 text-white"
            value={form.transporteOtro || ""}
            onChange={(e) => update("transporteOtro", e.target.value)}
          />
        )}
      </div>
      ...
      <div className="flex gap-4">
        ...
        <button
          onClick={next}
          disabled={
            form.transporte.length === 0 && !form.transporteOtro?.trim()
          }
